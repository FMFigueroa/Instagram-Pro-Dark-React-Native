...
const Likes = ({post}) => (
  <View style={{marginTop: 18}}>
    <Text style={{color: 'white', fontSize: 15}}>{post.likes} likes</Text>
  </View>
);
...
const CommentsSection = ({post}) => (
  <View style={{marginTop: 8}}>
    {!!post.comments.length && (
      <Text style={{color: '#2f2f2f', fontSize: 15}}> View
  ...
  {post.comments.map((comment, index) => (
    <View key={index} style={{marginTop: 10}} >
      <Text style={{color:'white',fontSize: 15, lineHeight: 24}}>
        ...
        <Text style={{fontWeight:'100', color:'#696969'}}>{'  '}{comment.comment}</Text>
      </Text> 
    </View>
  ))}
  </>
